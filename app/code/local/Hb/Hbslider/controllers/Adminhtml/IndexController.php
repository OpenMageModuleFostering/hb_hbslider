<?php
class Hb_Hbslider_Adminhtml_IndexController extends Mage_Adminhtml_Controller_Action
{
    protected function _initAction()
    {
        $this->loadLayout()->_setActiveMenu('hbslider/set_time')
      ...
      public function editAction()
      {
           ...
           $sliderModel = Mage::getModel('hbslider/hbslider')->load($sliderId);
           if($sliderModel->getId() || $sliderId == 0)
           {
           	Mage::register('slider_data',$sliderModel);
           	$this->loadLayout();
             $this->_setActiveMenu('hbslider/set_time');
             ...
             $this->_addContent($this->getLayout()
                  ->createBlock('hbslider/adminhtml_hbslider_edit'))
                  ->_addLeft($this->getLayout()
                  ->createBlock('hbslider/adminhtml_hbslider_edit_tabs')
              );
             $this->renderLayout();
           }
           else
       ...
       public function saveAction()
       {
       		if($data = $this->getRequest()->getPost())
       		{
       			try{
       				...
       				$image = Mage::getModel('hbslider/hbslider')
                ...
                $uploader->setAllowedExtensions(array('jpg','jpeg','gif','png'));
                   ...
                   if (isset($data['imagename']['value'])){
                      $data['imagename'] = $data['imagename']['value'];
       				...
       				$sliderModel = Mage::getModel('hbslider/hbslider');
          ...
          public function deleteAction()
          {
              if($this->getRequest()->getParam('id') > 0)
              {
                try
                {
                    $testModel = Mage::getModel('hbslider/hbslider');
        ...
        if(!is_array($slideIds)) {
            ...
            try {
                foreach ($slideIds as $slideId) {
                    $slide = Mage::getModel('hbslider/hbslider')->load($slideId);
                    $slide->delete();
                }
